...
const objLat = (obj) => {
    ...
    return `my name is ${obj.firstName.replace(/^./, obj.firstName[0].toUpperCase())} ${obj.lastName.replace(/^./, obj.lastName[0].toUpperCase())} I am ${obj.age} YO, and I love ${obj.hobby}.`
...
//  fullName and techf  ithe applicant has more than one year of Experience
...
if ( arr[i].yearsOfExperience > 1)
{
          ...
          out.push({fullName : fullName , tech: arr[i].tech})
}
}
return out
}
...
const classesAvg = (data) => {
    ...
    for (let j = 0 ; j < data.grades.classes.length;j++){
        let suum = 0
    for ( let k = 0 ; k< data.grades.classes.classScores.length ; k++){
suum=summ+classScores[k]

    }
    data.grades.classes.avg=suum/classScores.length
}}
    // write your code here
};
// -------------------------------------------------------------------------------------------------------
